...
function TwoDecimalsOnly(event, txtbox) {    
    var $this =document.getElementById(txtbox);
    ...
    if (getAssignedProjectsList) {
      for (var i = 0; i < getAssignedProjectsArray.length; i++) {
        if (sessionStorage.getItem('session_email') == getAssignedProjectsArray[i].AssignedEmail) {
          ...
          getAssignedProjectsList.innerHTML += '<div class="card"><div class="card-container"><h4><b>' + getAssignedProjectsArray[i].AssignedProject + '</b></h4><p><b>Assigned To: </b>' + getAssignedProjectsArray[i].AssignedEmail + '</p><p><b>Assigned By: </b>' + getAssignedProjectsArray[i].AssignedBy + '</p></div></div>';
        }
      }
    }
  }
      ...
      if (getAssignedTasksList) {
        for (var i = 0; i < getAssignedTasksArray.length; i++) {
          if (sessionStorage.getItem('session_email') == getAssignedTasksArray[i].AssignedTaskEmail) {
            getAssignedTasksList.innerHTML += '<div class="card"><div class="card-container"><h4><b>' + getAssignedTasksArray[i].AssignedTask + '</b></h4><p><b>Assigned By: </b>' + getAssignedTasksArray[i].AssignedTaskBy + '</p><p><b>Priority: </b>' + getAssignedTasksArray[i].AssignedTaskPriority + '</p><p><b>Description: </b>' + getAssignedTasksArray[i].AssignedTaskDesription + '</p><form id="formMarkTask" onsubmit="markCompleted();"><p>Total Hours : <input type="text" name="completed_hours" id="completed_hours" value="" onkeypress="TwoDecimalsOnly(event,this.id)"/></p><input type="hidden" name="mark_completed_task_id" id="mark_completed_task_id" value="' + getAssignedTasksArray[i].AssignedTaskId + '"/><input type="hidden" name="task_created_by" id="task_created_by" value="' + getAssignedTasksArray[i].AssignedTaskBy + '"/><input type="hidden" name="mark_completed_task" id="mark_completed_task" value="' + getAssignedTasksArray[i].AssignedTask + '"/><input type="hidden" name="task_price" id="task_price" value="' + getAssignedTasksArray[i].AssignedTaskPrice + '"/><input type="hidden" name="assigned_task_key" id="assigned_task_key" value="' + i + '"/><button id="assigned-task-done">Mark Completed</button></form></div></div>';
          }
        }
      }
    }
  ...
  loadtasks();
  function loadtasks(){
    var projname=document.getElementById('assign_project').value;
    var getProjectsArray = JSON.parse(localStorage.getItem("CreatedTasks"));
   
    const filteredArray = getProjectsArray.filter(obj => obj.ProjectName == projname);
    loadalltasks(filteredArray);
  }
  //loadalltasks();
function loadalltasks(filteredArray){
  if (localStorage.getItem('CreatedTasks')) {
   // var getProjectsArray = JSON.parse(localStorage.getItem("CreatedTasks"));
    var getProjectsList = document.getElementById("assign_task");
    getProjectsList.innerHTML='';
    if (getProjectsList) {
      for (var i = 0; i < filteredArray.length; i++) {
        if (sessionStorage.getItem('session_email') == filteredArray[i].TaskEmail) {
          getProjectsList.innerHTML += '<option  id="assign_task" value="' + filteredArray[i].TaskName + '">' + filteredArray[i].TaskName + '</option>';
        }
      }
    }
  }
